...
static Clr *tagscheme[LENGTH(tags)];
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]);
		drw_setscheme(drw, m->tagset[m->seltags] & 1 << i ? tagscheme[i] : (occ & 1 << i? scheme[SchemeSel] : scheme[SchemeNorm] ));
...
setup(void)
{
	...
	/* init tag selected colors */
	for (unsigned int i = 0; i < LENGTH(tags); i++) {
		tagscheme[i] = drw_scm_create(drw, tagselcolors[i], 3);
	}
